(function (global, factory) {
  ...
  class Shape {
    ...
    constructor(p5obj, vertices, startFrame, duration) {
      if (vertices.length <= 0) throw new Error("The vertex array must not be empty.");
  ...
  class AnimShapes {
    ...
    arc(id, duration, x, y, w, h, start, stop) {
      ...
      const createShapeVertices = () => {
        ...
        curves.forEach((curve, index) => {
          if (index === 0) shapeVertices.push(new Vertex(x + curve.x1 * rx, y + curve.y1 * ry));
          shapeVertices.push(
            new BezierVertex(
              x + curve.x2 * rx,
              y + curve.y2 * ry,
    ...
    triangle(id, duration, x1, y1, x2, y2, x3, y3) {
      const instance = this.getOrCreateShapeInstance_(
        id,
        () => {
          return [new Vertex(x1, y1), new Vertex(x2, y2), new Vertex(x3, y3), new Vertex(x1, y1)];
        },
        duration
      );
      instance.update();
