...
function drawAxis(layout, colour = "blue") {
  stroke(color(colour));
...
function drawYAxisTickLabels(min, max, layout, mapFunction, decimalPlaces) {
  ...
  for (i = 0; i <= layout.numYTickLabels; i++) {
    ...
    // Add tick label.
    text(value.toFixed(decimalPlaces), layout.leftMargin - layout.pad - 15, y);
...
function drawXAxisTickLabel(value, layout, mapFunction) {
  ...
  // Add tick label.
  text(value, x - 15, layout.bottomMargin + layout.marginSize / 2);
